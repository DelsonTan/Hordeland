    ...
    render() {
        ...
        if (this.props.eliminations.length > 0) {
            messages = this.props.eliminations.map((elimination) => {
                ...
                return (<span className="log" key={Math.floor(Math.random() * 100000)}>{attacker.name} 🔪<span className="elim">eliminated</span> <i className="fas fa-skull"></i> {target.name}<br/></span>)
            })
        }
        return (
            <div id="eliminations">
